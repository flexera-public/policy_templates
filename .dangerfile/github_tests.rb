...
def github_pr_missing_ready_label?(github)
  fail_message = ""
  fail_message = "**Github Pull Request**\nPull Request is missing `READY-FOR-REVIEW` label. Please add this label if this Pull Request is ready for review.\n\nPlease note that this warning may be a false positive if you've added the label after Dangerfile tests were run, since adding labels does not trigger them to run again. In these cases, simply ignore this warning." if !github.pr_labels.include?("READY-FOR-REVIEW")
