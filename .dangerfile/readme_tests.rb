...
def readme_api_permission_mismatch?(file, file_lines)
  ...
  derived_aws.each   { |p| missing_from_readme  << "AWS: `#{p}`"   unless readme_aws_perms.include?(p) }
  derived_azure.each { |p| missing_from_readme  << "Azure: `#{p}`" unless readme_azure_perms.include?(p) }
  derived_gcp.each   { |p| missing_from_readme  << "GCP: `#{p}`"   unless readme_gcp_perms.include?(p) }

  readme_aws_perms.each   { |p| missing_from_derived << "AWS: `#{p}`"   unless derived_aws.include?(p) }
  readme_azure_perms.each { |p| missing_from_derived << "Azure: `#{p}`" unless derived_azure.include?(p) }
  readme_gcp_perms.each   { |p| missing_from_derived << "GCP: `#{p}`"   unless derived_gcp.include?(p) }
