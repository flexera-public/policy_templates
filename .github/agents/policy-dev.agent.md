...
- `tags` — array of `"key=value"` strings; CCO uses these to route recommendations to the right billing center. (Not to be confused with the `info()` block's `tags` field, which is unrelated catalog-search metadata - see "Policy Template Anatomy" above.)
...
**`tags` field (required):** Every published and unpublished policy template must include a `tags` field in `info()` — a comma-separated string, with no spaces around the commas, listing every discrete use-case tag that applies (cloud provider, specific service, use case, recommendation type, internal Flexera use case, etc). This is a separate concept from the per-incident export `tags` field used for CCO billing-center routing (see "Total Potential Savings & Recommendations Integration" below) — the `info()` block `tags` field is catalog-search metadata, not resource tags.

- Every value used **must** already exist in the canonical list at [data/policy_tags/all_tags.json](../../data/policy_tags/all_tags.json). Do not invent a new tag ad hoc; if a genuinely new use case isn't covered, add it to `all_tags.json` first (as a `{"tag": "...", "description": "..."}` object, keeping the file pretty-printed and alphabetically/logically grouped like the rest of the list) and only then reference it from the template.
- List broader tags first (provider, then specific service), followed by more specific use-case tags. Include every tag that legitimately applies, even when tags overlap/appear redundant — e.g. a cross-family rightsizing policy should include both `Rightsizing` and `Cross-Family Rightsizing`.
- Always place `tags` as the **last** field in `info()`, after `deprecated`/`publish`/`hide_skip_approvals` if present.
- The `Dangerfile` enforces this field's presence, format, and validity against `all_tags.json` (see `policy_bad_tags?` in `.dangerfile/policy_tests.rb`) - a template with a missing, malformed, or unrecognized tag value will fail CI.
- **Meta parent policy templates must carry the same `tags` value as their child template.** Files generated by `tools/meta_parent_policy_compiler` get this automatically (the compiler copies the child's `tags` field into the generated parent). For the small number of manually-maintained meta parent files (not driven by `tools/meta_parent_policy_compiler/default_template_files.yaml`), copy the child's `tags` value by hand whenever the child's tags change.
- Adding or changing a template's `tags` field is a `.pt` file change like any other — it requires a version bump (PATCH is sufficient for a tags-only change) and a CHANGELOG entry.
...
- **Referenced directly by DSL** — a `datasource ... request do` block's `host`/`path`/`query`, or an `escalation` block (e.g. `param_cbi_endpoint` used as a `query`/`body_field` value) — instead of being processed by any script: create a dedicated `ds_x` / `js_x` datasource pair that trims the value, then replace every reference to `$param_x` (including as an argument to other `run_script` calls) with `$ds_x`. Name the wrapper datasource/script after the underlying concept, not by prefixing `param_` onto the datasource name - use `ds_cbi_endpoint` / `js_cbi_endpoint`, never `ds_param_cbi_endpoint` / `js_param_cbi_endpoint`. All datasources use the `ds_` prefix and all scripts use the `js_` prefix per the standard naming convention; `ds_param_x` is not a valid naming pattern regardless of what the wrapped parameter is named.
...
**Naming collisions for Category B wrapper datasources:** If the natural `ds_x` name (dropping the `param_` prefix from the parameter name) collides with an existing, unrelated datasource already present in the file, pick a different descriptive suffix that avoids the collision (e.g. `ds_effective_date_input` when a distinct `ds_effective_date` datasource already exists for a derived/computed value) rather than falling back to the `ds_param_x` pattern.
...
**Azure — `ds_azure_subscriptions`:** Use the sanitized `$ds_azure_endpoint` if the template's own `param_azure_endpoint` is genuinely free-text; if it declares `allowed_values` (the common case, to support Azure China), reference the raw `$param_azure_endpoint` directly instead (see Parameter Sanitization exclusion):
...
**Do not prefix a wrapper datasource/script name with `param_`.** A Category B sanitization wrapper for `param_x` must be named `ds_x` / `js_x` (dropping the `param_` prefix entirely, since `ds_` and `js_` are already the prefixes that identify the block type), never `ds_param_x` / `js_param_x`. This applies to any wrapper datasource regardless of purpose - `ds_param_x` is never a valid datasource name in this catalog.
...
**2. `info()` block** — Add `deprecated: "true"` as the last field before `tags` (the `tags` field, if present, should always remain the very last field in `info()`):
...
- **`tags` field** — must be present, comma-separated with no spaces, no empty/duplicate entries, and every value must exist in `data/policy_tags/all_tags.json`
...
1. Write `.pt` file (omit `publish` field unless user requests unpublished), including a `tags` field in `info()` using only values from `data/policy_tags/all_tags.json`
