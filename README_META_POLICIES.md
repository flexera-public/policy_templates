...
#### **Construct "Mapping" Format for Parameters**
```json
...
```json
      ...
      "param_exclusion_tag_key": "",
      "param_automatic_action": [],
      "param_api_wait": 5
```

#### **Construct "Name-Value" Format for Parameters**
```json
        {
          "name": "<param name>",
          "value": parent_policy.options.<param name>
        }
```
Example:
```json
...
#### **Construct "isEqual" statements for Parameters**
```javascript
      if (!_.isEqual(parent_policy.options.<param name>, existing_policies["formatted"][cur.aws_account_id]["options"]["<param name>"])) {
        should_update = true
      }
```
Example:
...
#### **Use Template to create New Meta Parent Policy**

The filename should be in the format of:
`<child_policy_template_name>_meta_parent.rb`

Replace the placeholders identified with `PLACEHOLDER` with the appropriate value.

<details><summary><b>Click to Expand</b> Meta Parent Policy Template</summary>

```ruby
name "__PLACEHOLDER_FOR_CHILD_POLICY_NAME__ Meta Parent"
rs_pt_ver 20180301
type "policy"
short_description "Applies and manages \"child\" [__PLACEHOLDER_FOR_CHILD_POLICY_NAME__](__PLACEHOLDER_FOR_CHILD_POLICY_GITHUB_URL__) Policies."
severity "low"
category "Cost"
tenancy "single"
...
script "js_format_self", type: "javascript" do
  ...
  formatted={
    ...
    "options": {
      ...
      "param_email": [],
      // All other child policy params excluding param_aws_account_number
      __PLACEHOLDER_FOR_CHILD_POLICY_PARAMETERS_MAPPING_FORMAT__
    }
  }
...
script "js_take_in_parameters", type: "javascript" do
  ...
  applied_policy_url_prefix = ui_url_prefix + "/policy/projects/" + rs_project_id + "/applied-policies/"
  incident_url_prefix       = ui_url_prefix + "/automation/incidents/projects/" + rs_project_id + "?incidentId="
  ...
  for (x=0; x<aws_account_ids.length; x++) {
    ...
    if (existing_policies.formatted[cur.aws_account_id] == undefined) {
      ...
      description = child_policy.name + " policy applied by [" + parent_policy.name + "](" + applied_policy_url_prefix + meta_parent_policy_id + ") to AWS Account **" + cur.aws_account_name +"** (`" + cur.aws_account_id + "`).  " + child_policy.short_description
      options = [
        // param_email should always be an empty list
        {
          "name": "param_email",
        ...
        // All other child policy params excluding param_aws_account_number and param_email
        __PLACEHOLDER_FOR_CHILD_POLICY_PARAMETERS_NAME_VALUE_FORMAT__
      ]
      ...
      __PLACEHOLDER_FOR_CHILD_POLICY_PARAMETERS_ISEQUAL_FORMAT__
