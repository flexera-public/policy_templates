...
- The policy checks the Spot CCO Export integration status by querying the Spot API (`/ocean/flexeraCcoIntegration`). If the integration is not configured, the incident report includes pre-filled `curl` and PowerShell commands with the correct bill connect ID, Flexera org ID, and zone — requiring only a Flexera refresh token and Spot API token to complete the setup.
