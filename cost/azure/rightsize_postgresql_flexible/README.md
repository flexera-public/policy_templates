...
- An Azure PostgreSQL Flexible Server is considered **unused** if the maximum number of active connections observed during the lookback period does not exceed the *Unused Server Connection Threshold* parameter (default: 1). The default of 1 accounts for the internal connection that Azure maintains on every running PostgreSQL Flexible Server; increase this value if your environment maintains additional persistent background connections.
...
- *Unused Server Connection Threshold* - The maximum number of active connections a server can have and still be considered unused. Defaults to 1 to account for the internal connection that Azure always maintains on running PostgreSQL Flexible Servers. Increase this value if your environment maintains additional persistent background connections (e.g. monitoring agents).
