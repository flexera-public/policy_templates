...
- *Unused Server Connection Threshold* - The maximum average number of active connections a server can have over the lookback period and still be considered unused. Uses the average rather than the maximum to avoid false positives from brief connection spikes during provisioning or maintenance. Azure PostgreSQL Single Servers maintain several internal connections at all times (connection pooling, monitoring, HA processes, etc.); the default of 10 is chosen to be above the typical idle baseline. Run the policy once and inspect the `Average Active Connections` column to determine the appropriate threshold for your environment.
