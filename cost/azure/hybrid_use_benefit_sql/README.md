...
- The `Estimated Monthly Savings` for **SQL Elastic Pools**, **SQL Databases**, and **SQL Managed Instances** is calculated using the resource's vCore count (`SKU Capacity`) and the SQL Server license price per vCPU from the [Azure Retail Prices API](https://learn.microsoft.com/en-us/rest/api/cost-management/retail-prices/azure-retail-prices). The formula is: `license_price_per_vcpu x vcpu_count x 730`.
- The `Estimated Monthly Savings` for **SQL Virtual Machines** is calculated using the SQL Server edition (`Image SKU`) and the vCPU count derived from the underlying Azure VM size. The vCPU count is parsed from the VM size name (e.g., `Standard_D4s_v3` → 4 vCPUs). The formula is: `license_price_per_vcpu x vcpu_count x 730`.
