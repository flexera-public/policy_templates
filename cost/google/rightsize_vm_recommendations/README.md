...
The policy includes the estimated monthly savings. The estimated monthly savings is recognized if an underutilized VM instance is downsized or if an idle VM instance is stopped or deleted. Savings estimates are provided by the Google Recommender service. The incident message detail includes the sum of each resource *Estimated Monthly Savings* as *Potential Monthly Savings*.
