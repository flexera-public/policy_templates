...
This policy reports on any existing rightsize or delete recommendations for VM instances generated by the Google Recommender service. The user can then choose to downsize or delete the VM instance if desired. Optionally, the user can filter results by label, project ID/name, or region.
...
- `google.compute.instance.IdleResourceRecommender`: Checks if a VM instance is idle and recommends deletion when appropriate.
...
The policy includes the estimated monthly savings. The estimated monthly savings is recognized if an underutilized VM instance is downsized or if an idle VM instance is deleted. Savings estimates are provided by the Google Recommender service. The incident message detail includes the sum of each resource *Estimated Monthly Savings* as *Potential Monthly Savings*.
