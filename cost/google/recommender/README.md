...
This policy template reports on any existing cost optimization recommendations generated by the Google Recommender service. Optionally, the user can filter results by recommendation type, project ID/name, or region.

**NOTE:** This policy template is only recommended if you specifically need a consolidated view of several recommenders in a single report and for recommenders that do not have a dedicated policy template. Otherwise, it is recommended that you use one or more of the following policy templates instead to ensure that your recommendations are scraped into the Optimization dashboard:
...
- *[Google Rightsize Cloud SQL Recommender](https://github.com/flexera-public/policy_templates/tree/master/cost/google/rightsize_cloudsql_recommendations)* - Idle Cloud SQL Instance
...
The following recommenders are currently supported:

- Account Security
- Cloud SQL Instance Out-of-Disk
...
- Product Suggestion - Compute
- Overprovisioned Cloud SQL Instance
- VM Machine Type
- Unattended Project
