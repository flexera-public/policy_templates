...
This Policy Template reports any Committed Use Discount Recommendations generated by Google at the project level, billing account level, or both. The user can adjust which recommendations are reported via policy parameters.
...
- *Recommendation Scope* - Whether to fetch project-level recommendations, billing account-level recommendations, or both. Defaults to `Project` for backwards compatibility. When `Billing Account` or `Both` is selected, the Google credential requires additional billing account-level IAM permissions (see Prerequisites).
...
- [**Google Cloud Credential**](https://docs.flexera.com/flexera-one/automation/automation-administration/managing-credentials-for-policy-access-to-external-systems/provider-specific-credentials#google) (*provider=gce*) which has the following:
  ...
  † Only required when `Recommendation Scope` is set to `Billing Account` or `Both`. Both `billing.accounts.list` and `recommender.usageCommitmentRecommendations.list` must be granted at the billing account level in Google Cloud Console (e.g. via `roles/billing.viewer` and `roles/recommender.computeCommitmentViewer` on the billing account), not just at the project level.
...
- [Cloud Billing API](https://console.cloud.google.com/flows/enableapi?apiid=cloudbilling.googleapis.com) *(only required when `Recommendation Scope` is set to `Billing Account` or `Both`)*
