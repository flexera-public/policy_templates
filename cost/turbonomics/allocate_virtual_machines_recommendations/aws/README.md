...
- [**Turbonomic Credential**] The policy requires basic Turbonomic authentication credentials, ensure your Turbonomic credentials meet the following OAuth 2.0 authentication criteria:
  ...
  - Specify a unique audience value (spec.properties.global.oauth2.audience) in the CR file. By default, this policy template has defualt value of `flexera` for audience parameter, but you can customize it with any unique string value and use that value.
  - Utilize the Turbonomic API to create an OAuth 2.0 client, configuring it with `role:OBSERVER` and using `client_secret_basic` for `client authentication methods`.
  - Keep the provided client id and client secret by the API for adding the Turbonomic credentials and ensure the audience value aligns with the Turbonomic Audience parameter in the policy template.
