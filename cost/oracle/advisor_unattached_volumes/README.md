...
This policy template reports on any existing unattached volume recommendations generated by Oracle Cloud Advisor. Optionally, this report can be emailed.

### How It Works

This policy template reports the following Oracle Cloud Advisor recommendations:

- `cost-management-boot-volume-attachment`: Checks if a boot volume is unattached and recommends deleting it.
- `cost-management-block-volume-attachment`: Checks if a block volume is unattached and recommends deleting it.
...
- *Volume Types* - The types of volumes to provide recommendations for.
...
For example if a user selects the "Delete Unattached Volumes" action while applying the policy template, all the resources that didn't satisfy the policy condition will be deleted.
...
- [**Oracle Credential**](https://docs.flexera.com/flexera-one/automation/automation-administration/managing-credentials-for-policy-access-to-external-systems/provider-specific-credentials#automationadmin_3335267112_1121578) (*provider=oracle*) which meets the below requirements:
  ...
  - `Allow group <group> to manage volume-family in tenancy`*
