# Oracle Cloud Advisor: Burstable Virtual Machines

## What It Does

This policy template reports on any existing burstable virtual machine recommendations generated by Oracle Cloud Advisor. Optionally, this report can be emailed.

### How It Works

This policy template reports the following Oracle Cloud Advisor recommendations:

- `cost-management-compute-host-burstable`: Checks if a virtual machine can be resized to a burstable machine type.
...
For example if a user selects the "Resize Virtual Machines" action while applying the policy, all the resources that didn't satisfy the policy condition will be resized.
...
- Resize VM instances to burstable machine type after approval
