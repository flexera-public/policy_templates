...
- *Alert ID* - The unique identifier for the alert, used to associate the webhook with a specific alert in the system.
- *Subscription ID* - The unique identifier for the subscription, required to link the webhook to the appropriate subscription.
- *Alert Data* - Key-value pairs that define the payload for the webhook. Examples: 'condition=greater-than', 'threshold=90' specify the alert conditions.
- *Automatic Action* - Actions triggered automatically when the alert is raised. For example, triggering an Alert Subscription Webhook.
...
Additionally, ensure that you have created Alert and Subscription in Flexera One:
