...
- *Look Back Period (Days)* - How many days back to look at network billing data to assess if an AWS NAT Gateway is idle.
...
For example, if a user selects the "Delete Idle NAT Gateways" action while applying the policy template, all the gateways that didn't satisfy the policy condition will be deleted.
...
- Delete idle gateways after an approval
