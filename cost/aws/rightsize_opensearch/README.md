...
**Underutilized domains:** The `Estimated Monthly Savings` is estimated by multiplying the domain's monthly instance-only CCO cost by the fraction of resources saved — the greater of: (1) 1 minus the ratio of the recommended instance type's vCPU count to the current instance type's vCPU count, or (2) 1 minus the ratio of the recommended instance type's memory to the current instance type's memory. Using the larger of the two ratios better reflects savings for memory-optimized instance families. For example, downsizing from an 8-vCPU/64 GiB instance to a 4-vCPU/32 GiB instance yields an estimated 50% savings. Instance-only costs include billing charges for compute instance hours only (excluding EBS storage, Extended Support, data transfer, and other non-scalable charges) to improve accuracy.
