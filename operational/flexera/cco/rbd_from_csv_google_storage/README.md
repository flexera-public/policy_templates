# Rule-Based Dimensions from CSV - Google Cloud Storage

## What It Does

This policy template creates and updates Flexera Rule-Based Dimensions (RBDs) from a CSV file hosted in Google Cloud Storage. The CSV uses a `||DIVIDER||` column to separate rule-dimension columns (conditions) from RBD-output columns (values). The policy resolves dimension display names to API IDs at runtime, validates CSV data quality, and uses sentinel rules to safely merge generated rules with manually-created or externally-managed rules.

## How It Works

- The policy lists objects in the specified GCS bucket matching the configured prefix and selects the latest file by lexicographic sort (or a specific file if overridden).
...
- *Bucket Name* - The name of the Google Cloud Storage bucket containing the CSV file.
- *File Prefix* - Only objects whose name starts with this prefix will be considered. Default: `rbd_`.
- *File Override* - If non-empty, forces use of this exact object name instead of automatically selecting the latest file by name.
...
- [__Google Cloud Credential__](https://docs.flexera.com/flexera/EN/Automation/ProviderCredentials.htm#automationadmin_4083446696_1121577) (*provider=gce*) which has the following permissions:
