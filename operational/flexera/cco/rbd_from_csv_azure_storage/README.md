# Rule-Based Dimensions from CSV - Azure Storage

## What It Does

This policy template creates and updates Flexera Rule-Based Dimensions (RBDs) from a CSV file hosted in Azure Blob Storage. The CSV uses a `||DIVIDER||` column to separate rule-dimension columns (conditions) from RBD-output columns (values). The policy resolves dimension display names to API IDs at runtime, validates CSV data quality, and uses sentinel rules to safely merge generated rules with manually-created or externally-managed rules.

## How It Works

- The policy lists blobs in the specified Azure Blob Storage container matching the configured prefix and selects the latest file by lexicographic sort (or a specific file if overridden).
...
- *Storage Account Name* - The name of the Azure Storage account containing the CSV file. Example: `mystorageaccount`.
- *Container Name* - The name of the Azure Blob Storage container containing the CSV file.
- *File Prefix* - Only blobs whose name starts with this prefix will be considered. Default: `rbd_`.
- *File Override* - If non-empty, forces use of this exact blob name instead of automatically selecting the latest file by name.
