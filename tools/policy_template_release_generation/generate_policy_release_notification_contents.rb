...
# Reads changed CHANGELOG.md and .pt files from the latest master commit and posts
# a formatted Microsoft Teams webhook notification summarizing policy template updates.
...
if $PROGRAM_NAME == __FILE__
  ...
  if all_notification_content_array.length > 0
    ...
    Net::HTTP.start(webhook.host, webhook.port, use_ssl: webhook.scheme == 'https') do |http|
