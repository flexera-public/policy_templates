#!/usr/bin/env python3
"""
Fetches all Google Compute Engine machine types via the Compute Engine API
and merges them with manual supplementary data (supersession flags).

Produces: data/google/google_compute_instance_types.json
Usage: python3 google_compute_instance_types.py
...
def list_all_machine_types():
    """Fetch all machine types from all GCP zones using the Compute Engine API."""
    ...
    # pylint: disable=no-member  # machineTypes() is dynamically generated by the Google API Discovery client
    ...
    while request is not None:
        ...
        # Collect machine types from every zone in the aggregated response
        ...
        request = compute_service.machineTypes().aggregatedList_next(  # pylint: disable=no-member
...
def main():
    ...
    for item in machine_types:
        ...
        if name in data:
            # Accumulate all zones that offer this machine type
